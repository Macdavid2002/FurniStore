...
export const Search = () => {
  ...
  return (
    <div className="px-4 py-36 md:p-24 md:py-64">
      ...
      />
      {filterItems(data.data[0])}
      {filterItems(data.data[1])}
      {filterItems(data.data[2])}
      {filterItems(data.data[3])}
      {filterItems(data.data[4])}
      {filterItems(data.data[5])}
      {filterItems(data.data[6])}
      {filterItems(data.data[7])}
      {filterItems(data.data[8])}
    </div>
  );
};
